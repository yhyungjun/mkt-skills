...
> **출처**: axacademy-landing 2기 (공개 50만 `/` · 프라이머 클럽 40만 `/primerclubac`).
...
/** 비공개 변형 전용 경로 (추측 어려운 슬러그 + noindex). */
export const PRIMER_SLUG = "primerclubac";
...
```tsx
// app/primerclubac/page.tsx — 비공개(primer), 검색 노출 차단
...
(variant별로 좌석을 나누고 싶으면 카운터 쿼리에 `.eq("variant", v)` 추가. axacademy는 공유 풀.)
