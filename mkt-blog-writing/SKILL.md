...
### 글 슬러그 + 카테고리 지정 (발행 준비값 확정)

CTA까지 확정되면 발행 준비값으로 **카테고리**와 **슬러그**를 함께 확정한다. 이 둘은 이후 단계에서 그대로 재사용된다.

**카테고리 (4종 중 1개 — 사용자 확인, 추론 금지)**
- 기업교육 소식 / AX 인사이트 / 도입사례 / AI 도구·가이드 (조코딩AX 무료진단 캠페인이면 무료 진단).
- `blog_type`(INFO/PURCHASE/PORTFOLIO/DAILY)과 **다른 축**이다. 반드시 사용자에게 확인.
- 확정된 카테고리는 ① **CTA 버튼 카피·색**(위 표) ② **Step 6.5 썸네일 태그색** ③ **Step 7 inblog 카테고리 태그(`add-tags`)** 에 공통 적용된다.

**슬러그 (영문 케밥 — SEO URL)**
- `selected_title`/`selected_keyword`를 영문 케밥으로 변환: 한글→의미 영문(음역 아님), 모두 소문자, 공백→`-`, 특수문자 제거.
- 예: "비개발자 바이브코딩 사례" → `vibe-coding-nondev-case`
- 사용자에게 확인받아 확정하고, Step 7 `inblog posts create --slug`에 그대로 사용.
- ⚠️ 슬러그에 **실제 고객사명·실명·비공개 서비스명** 등 노출 금지(익명화 대상이면 특히).
...
**배경 이미지형(실사 풀배경 V2)을 기본·우선 양식으로 고정**한다.

### 양식 우선순위 (고정)

**① [기본] 배경 이미지형 — 실사 풀배경 V2 템플릿**
...
3. **기본**: 배경 이미지형 V2로 제작 — 사용자 실사 사진 + 히어로 카피(18~25자, 2줄) 합성 → `bonda-use/thumbnail/{N}-...png`.
